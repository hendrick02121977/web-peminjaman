...
function createData() {
  ...
  database.ref("Data_Peminjam").push(data);
  namaV.value = "";
  nohpV.value = "";
  kelasV.value = "";
  jamV.value = "";
...
// Read Data
database.ref("Data_Peminjam").on("value", ambildata);
...
//show data edit
function editRow(id) {
  database.ref("Data_Peminjam/" + id).on("value", function (snapshoot) {
...
function updateData() {
  ...
  database.ref("Data_Peminjam/" + idV.value).update(data);
  updateData.setAttribute("data-bs-dismiss", "modal");
}

function deleteRow(id) {
  database.ref("Data_Peminjam/" + id).remove();
}
